...
// PhEyeConfiguration holds connection settings for a direct PhEye model call.
type PhEyeConfiguration struct {

	// ModelPath is the path to the PhEye model file.
...
// PhEyeFilter configures detection and handling of entities using a direct PhEye model call.
type PhEyeFilter struct {
	BaseFilter

	// PhEyeConfiguration holds the connection configuration for the PhEye model.
	PhEyeConfiguration PhEyeConfiguration `json:"pheyeConfiguration,omitempty"`
	// PhEyeFilterStrategies defines how to handle identified entities.
...
type Identifiers struct {
	...
	// PhEye configures entity detection using a direct PhEye model call.
	// Multiple PhEye filters may be configured, each pointing to a different model instance.
